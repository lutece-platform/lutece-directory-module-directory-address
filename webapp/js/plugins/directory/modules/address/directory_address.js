...
function checkAddress( id_entry )
{
	...
	{
		$.getJSON(
				document.getElementsByTagName('base')[0].href + "jsp/site/plugins/directory/modules/address/SearchAddress.jsp",
				{
...
function displayResult( data, id_entry )
{
	...
	div_results.innerHTML = data.html;
	document.getElementById( id_entry + '_button_check' ).disabled = '';
}
...
function selectAddress( id_entry, code_address )
{
	...
	div_results.innerHTML = '';
	$.getJSON(
			document.getElementsByTagName('base')[0].href + "jsp/site/plugins/directory/modules/address/FindAddressInfos.jsp",
			{
